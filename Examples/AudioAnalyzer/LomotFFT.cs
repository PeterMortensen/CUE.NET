...
{
    // Taken from https://raw.githubusercontent.com/billism1/KeyboardAudio/master/KeyboardAudio/LomontFFT.cs as of 29.09.2015
    ...
    public class LomontFFT
    {
        ...
        public void RealFFT(double[] data, bool forward)
        {
            ...
            double sign = -1.0; // assume inverse FFT, this controls how algebra below works                        
            if (forward)
            { // do packed FFT. This can be changed to FFT to save memory                                        
                ...
                if (A != 1)
                {
                    double scale = Math.Pow(2.0, (A - 1) / 2.0);
                    for (int i = 0; i < data.Length; ++i)
                        data[i] *= scale;
                }
            }

            double theta = B * sign * 2 * Math.PI / n;
            double wpr = Math.Cos(theta);
            double wpi = Math.Sin(theta);
            double wjr = wpr;
            double wji = wpi;
            ...
            {
                double temp = data[0]; // unpack the y0 and y_{N/2}, then invert FFT                                
                ...
                {
                    double scale = Math.Pow(2.0, -(A + 1) / 2.0) * 2;
                    for (int i = 0; i < data.Length; ++i)
                        data[i] *= scale;
                }
            }
